...
setwd ("/Users/fabianyii/Desktop/EyeQue Study/pvt/Plots") #set working "data" as working directory
library("readxl") #attach readxl package
eyeque <- data.frame(read_excel("/Users/fabianyii/Desktop/EyeQue Study/pvt/Data/eyeque.xlsx")) #read excel data into R
...
for(i in 1: length(unique$name)){
  ...
  plot(d$m, d$j0, main = paste0(unique$name[i], "(",unique$eye[i],")", ":", " ", "J0, M"), 
       ...
       abline (h = mean(d$j0), v = mean(d$m), lty = 1, lwd = 0.3),
  ...
  plot(d$m, d$j45, main = paste0(unique$name[i], "(",unique$eye[i],")", ":", " ", "J45, M"),
       ...
       abline (h = mean(d$j45), v = mean(d$m), lty = 1, lwd = 0.3),
       pch = 19, cex = 1.2, col = brewer.pal(10, 'RdYlBu'))
  ...
  plot(d$j0, d$j45, main = paste0(unique$name[i], "(",unique$eye[i],")", ":", " ", "J45, J0"),
       ...
       abline (h = mean(d$j45), v = mean(d$j0), lty = 1, lwd = 0.3),
       pch = 19, cex = 1.2, col = brewer.pal(10, 'RdYlBu'))
  ...
  plot(d$sphere, d$cyl, main = paste0(unique$name[i], "(",unique$eye[i],")", ":", " ", "Cylinder, Sphere"),
       ...
       abline (h = mean(d$cyl), v = mean(d$sphere), lty = 1, lwd = 0.3),
       pch = 19, cex = 1.2, col = brewer.pal(10, 'RdYlBu'))
