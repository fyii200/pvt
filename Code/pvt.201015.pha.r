...
#  for work on EyeQue's PVT device
...
rm (list=ls())
#setwd ("/Users/fabianyii/Desktop/EyeQue Study/pvt/Plots")  # Fabian
...
d <- data.frame(read_excel("data/eyeque.xlsx"))   # Note how I do this -- we only need to set (and change!) the path once, between our systems.

mfunction   <- function (s, c) {round(s + c/2, digits = 2)}         # here, the function is neat, but not really necessary
j0function  <- function (c, a) {round(-c/2*cos(2*a*pi/180), digits = 2)}
j45function <- function (c, a) {round(-c/2*sin(2*a*pi/180), digits = 2)}

d$m <-   mfunction(   d$sphere, d$cyl)  #sphero-cylindrical notation to power vectors
d$j0 <-  j0function(  d$cyl,    d$axis)
d$j45 <- j45function( d$cyl,    d$axis)
...
for(i in 1: length(ID$name)){
  ...
  plot(d$m, d$j0, main = paste0(unique$name[i], "(",unique$eye[i],")", ":", " ", "J0, M"),   #  this (40-46) should be packed into a function :)
       xlab = "M (D)", ylab = "J0 (D)", 
