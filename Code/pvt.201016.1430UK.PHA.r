...
for(i in 1: length(ID$name)){
 ...
 # e <- d [idx[1:length(idx)], 1:9]
  ...
  e <- d [idx, ]  #  Fabian -- this is much easier than your solution, and will work no matter how many tests there are.
 ...
 # e$y <- 0
  ...
  #plot(e$x, e$y, cex=0.1, pch=3, xlab="90°", ylab="180°", 
  plot(0, 0, cex=0.1, pch=3, xlab="90°", ylab="180°", 
       main = paste0(ID$name[i], "(",ID$eye[i],")", ":", " ", "Axis"), 
       bty="n", xaxt="n", yaxt="n") 
  
  abline(h=0, v=0, lwd=0.5)
  sl <- c(row.names(data.frame(e$axis)))
  
  #for (val in sl) {
  #  abline(a=0, b= e$slope[val], lty=1, lwd=1, col="red")}
  
  for (i in 1:length( e$slope ) ) {
    abline(a=0, b=e$slope[i], lty=1, lwd=1, col="red") }
...
# =============================    good thought to save 'old' snippets!  "Use Code > Uncomment/Comment Lines"
